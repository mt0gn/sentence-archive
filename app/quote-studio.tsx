...
const textLineStyleOptions: Array<{ value: TextLineStyle; label: string }> = [
  ...
  { value: "horizontal-short", label: "가로 실선" },
  { value: "horizontal-double", label: "가로 이중선" },
  { value: "horizontal-dotted", label: "가로 점선" },
  { value: "horizontal-dashed", label: "가로 파선" },
];
...
function normalizeRule(value: Partial<SyntaxRule>): SyntaxRule {
  ...
  const normalizedLineStyle = (["none", "vertical-solid", "vertical-double", "vertical-dotted", "vertical-dashed", "horizontal-short", "horizontal-double", "horizontal-dotted", "horizontal-dashed"] as string[]).includes(String(value.lineStyle))
  ...
  return {
    ...
    highlightColor: value.highlightColor || "transparent",
...
function normalizeLineStyle(value: unknown): TextLineStyle {
  ...
  if (requested === "horizontal-long") return "horizontal-short";
  return (["none", "vertical-solid", "vertical-double", "vertical-dotted", "vertical-dashed", "horizontal-short", "horizontal-double", "horizontal-dotted", "horizontal-dashed"] as string[]).includes(requested) ? requested as TextLineStyle : "none";
}
...
function ruleClassNames(rule: SyntaxRule) {
  const lineClasses = rule.role === "dialogue" ? [] : [`line-${rule.lineStyle}`, `line-position-${rule.linePosition}`];
...
function ruleStyle(rule: SyntaxRule): CSSProperties {
  return {
    ...
    backgroundColor: rule.highlightColor === "transparent" ? undefined : rule.highlightColor,
...
function PreviewText({ text, rules, mosaicTerms, paragraphIndent = false, directMarks = [], flowBlocks = [], paragraphMarks = [], baseOffset = 0, layout }: { text: string; rules: SyntaxRule[]; mosaicTerms: string[]; paragraphIndent?: boolean; directMarks?: DirectTextMark[]; flowBlocks?: FlowBlock[]; paragraphMarks?: ParagraphMark[]; baseOffset?: number; layout?: LayoutSettings }) {
  ...
  paragraphs.forEach((paragraph, index) => {
    ...
    const dialogueRule = parseSegments(paragraph.text, rules).find((segment) => segment.rule?.role === "dialogue")?.rule;
...
export default function QuoteStudio() {
  ...
  function renderToolPanel() {
    ...
    if (activeTool === "rules") return <div className="tool-content">
      ...
      <div className="control-card paragraph-review-card">
        ...
        <p className="helper-note">원문에 기호를 추가하지 않습니다. 빈 줄로 나뉜 문단을 골라 역할과 표현만 저장합니다.</p>
    ...
    if (activeTool === "edit") return <div className="tool-content">
      ...
      <div className="control-card flow-block-card">
        <div className="history-heading"><span className="field-label">문단 구분 장식</span><small>고정 SVG · 색상 변경 가능</small></div>
        <p className="helper-note">커서가 있는 문단 앞에 하나의 장식 블록을 넣은 뒤 모양을 고릅니다. 원문 문자와 페이지 경계 기능은 이곳에서 다루지 않습니다.</p>
      ...
      <div className="control-card free-photo-card">
        ...
        {["bubble", "messenger"].includes(layout.mode) ? <>
          ...
          <p className="helper-note">사진은 모든 비메신저 테마에서 캔버스 위를 자유롭게 이동합니다.</p>
    ...
    if (activeTool === "privacy") return <div className="tool-content">
      ...
      <div className="control-card word-search-card">
        ...
        {wordSearch.trim() && <>
          ...
          <p className="helper-note">여기서 해제한 위치는 익명화와 단어 교체에서 함께 제외되며 파란 마킹도 사라집니다.</p>
        </>}
      </div>
      <div className="control-card mosaic-control-card">
        ...
        <p className="helper-note">원문과 수정본의 글자는 바꾸지 않습니다. 등록된 단어를 누르면 위의 통합 검색에서 각 위치를 확인할 수 있습니다.</p>
    ...
    if (activeTool === "design") {
      ...
      return <div className="tool-content">
        ...
        <div className="control-card page-display-card">
          ...
          <label>페이지 번호 형식<input value={layout.pageNumberPattern} onChange={(event) => setLayout((current) => ({ ...current, pageNumberPattern: event.target.value }))} placeholder="{current:02} / {total:02}" /></label>
          <p className="helper-note"><code>{'{current}'}</code>는 현재 번호, <code>{'{total}'}</code>은 전체 페이지입니다. 예: <b>{'{current}장'}</b></p>
          <div className="current-page-display"><b>현재 페이지 개별 설정</b>{["classic", "bubble"].includes(layout.mode) && <label>제목<input value={layout.pageTitleOverrides[visiblePage.id] || ""} onChange={(event) => updateCurrentPageDisplay("title", event.target.value)} placeholder={layout.pageTitle || projectTitle || "기본 제목 사용"} /></label>}<label>번호 문구<input value={layout.pageNumberOverrides[visiblePage.id] || ""} onChange={(event) => updateCurrentPageDisplay("number", event.target.value)} placeholder={formatPageNumberPattern(layout.pageNumberPattern, currentPage + 1, displayPages.length)} /></label><button className="secondary-button full-button" onClick={() => setLayout((current) => { const titles = { ...current.pageTitleOverrides }; const numbers = { ...current.pageNumberOverrides }; delete titles[visiblePage.id]; delete numbers[visiblePage.id]; return { ...current, pageTitleOverrides: titles, pageNumberOverrides: numbers }; })}>현재 페이지 개별 설정 지우기</button></div>
        </div>
  ...
  return <main className={`quote-studio mobile-${mobileView} ${leftCollapsed ? "left-is-collapsed" : ""} ${rightCollapsed ? "right-is-collapsed" : ""}`} style={rootStyle}>
    ...
    {settingsOpen && <div className="modal-backdrop" onMouseDown={() => setSettingsOpen(false)}><section className="settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-title" onMouseDown={(event) => event.stopPropagation()}><div className="modal-header"><div><span className="eyebrow">INTERFACE</span><h2 id="settings-title">UI 안내</h2></div><button className="icon-button" aria-label="설정 닫기" onClick={() => setSettingsOpen(false)}>×</button></div><div className="control-card interface-note"><span className="interface-accent-swatch" /><div><b>{APP_NAME} Blue</b><p>작업 UI는 #6694EA를 중심으로 통일됩니다. 결과물의 색상 설정과는 별개입니다.</p></div></div><div className="font-license-note"><b>웹글꼴 안내</b><p>배포 가능한 글꼴 파일을 프로젝트에 포함했습니다. 네트워크 연결 없이도 미리보기와 이미지 저장에 같은 글꼴을 사용합니다.</p></div><button className="primary-button full-button" onClick={() => setSettingsOpen(false)}>확인</button></section></div>}
